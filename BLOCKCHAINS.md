...
Starknet uses the **`starknet_*` JSON-RPC protocol** (`starknet_blockNumber`, `starknet_getBlockWithTxHashes`, etc.). ChainCheck's `StarknetProtocol` adapter handles this natively. Block numbers are plain integers (not hex). Since v0.14 ("Grinta", September 2025) Starknet produces blocks every ~4–6 s under decentralized Tendermint sequencing (previously ~30 s) — the bundled profile polls at 5 s with a 60 s stale threshold to absorb the network's documented block-time jitter and sequencer stalls. Finality stages: `PRE_CONFIRMED` (v0.14+) → `ACCEPTED_ON_L2` (default, ~seconds) → `ACCEPTED_ON_L1` (hours). Enabling `get-finalized-blocks` maps the `finalized` tag to the `l1_accepted` block_id (JSON-RPC spec ≥ 0.8 nodes required); it is off by default because public gateways still run older specs. On every poll ChainCheck also issues `starknet_syncing`: `false` means synced, a sync-status object yields a blocks-behind count that drives the `SYNC_LAG` anomaly. Gateways that don't implement the method (JSON-RPC error `-32601`) are simply treated as providing no health information — no false anomaly. WebSocket subscriptions (`starknet_subscribeNewHeads`) are supported on Pathfinder and Juno nodes; HTTP polling is used as fallback.
...
Cosmos SDK chains use CometBFT's **HTTP GET RPC** (`GET /status`, `GET /block?height=N`) instead of JSON-RPC POST. WebSocket subscriptions use CometBFT's `subscribe` method with the query `tm.event='NewBlock'`. Block hashes are uppercase hex without `0x` prefix (stored lowercase in ChainCheck). BFT consensus provides instant finality — every committed block is final, so `get-safe-blocks` and `get-finalized-blocks` should be `false`. The WS URL must include the `/websocket` path suffix (e.g., `wss://my-node:26657/websocket`). Node health comes from `/status` → `sync_info.catching_up` (a catching-up node opens a `SYNC_LAG` anomaly), and the CometBFT version from `node_info.version` (condensed to e.g. `CometBFT/v0.38.11`) for fleet version-skew detection. Since the head request is the same `/status` call, identical GET requests within one poll are deduplicated — the probes add no extra round-trips. The reported latency for GET protocols is the round trip of the head request, not the sum of all sequential calls.
